...
export function ScorePanel({
  ...
  const scoreSize = isPhone ? 'text-2xl' : isLandscape ? 'text-xl' : 'text-[40px] md:text-5xl';
  ...
  return (
    <HudPanel
      glow
      className={`p-3 md:p-4 min-w-0 ${isPhone ? 'w-full max-w-none' : 'w-full max-w-[320px]'} ${className}`}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          className="shrink-0 h-9 w-9 flex items-center justify-center rounded-md text-cyan-300/80 hover:text-cyan-200 transition-colors"
